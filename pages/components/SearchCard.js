...
const SearchCard = ({ data, user, cid }) => {
    ...
    if (confirm.doc !== '' && confirm.startTime !== '') {
      async function confirmRouter() {
        ...
        if (confirm.docid !== undefined && confirm.startTime !== undefined) {
          data.map((doc) => {
            if (doc.id === confirm.docid) {
              setCid(doc.data().clinicId);
            }
          });
  ...
  return (
    <div>
      {data.length > 0 ? (
        <p className='my-5 font-bold text-2xl px-10'>Search Results:</p>
      ) : (
        ''
      )}
      <div className='flex w-full overflow-x-auto px-10'>
        {data[0] ? (
          <div className='flex'>
            {data.map((doc) => (
              <>
                <div className='flex flex-col justify-center items-center w-96 py-1 px-1 mx-3 bg-white border-2 border-bgg rounded-sm mb-2'>
                  <div className='flex'>
